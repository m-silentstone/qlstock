...
for stock_code in allstockcode_array:
    ...
    for index, row in stock_price_df.iterrows():
        ...
        stock_price_df.loc[index, 'spin']=''
        if upline > change * 2.5 and upline/float(row['close']) > 0.03:
            stock_price_df.loc[index, 'spin'] += 'U'
        if downline > change * 2.5 and downline/float(row['close']) > 0.03:
            stock_price_df.loc[index, 'spin'] += 'D'
