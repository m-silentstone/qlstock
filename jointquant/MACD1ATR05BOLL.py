...
def statistics(code, stock_price_df, context):
    ...
    if (stock_price_df.at[stock_price_df.index[-1], 'MACD'] > 0 and \
        ...
        atr_plus_price = stock_price_df.at[stock_price_df.index[-1], 'CLOSE'] + 0.5 * stock_price_df.at[
            stock_price_df.index[-1], 'ATR']
        atr_min_price = stock_price_df.at[stock_price_df.index[-1], 'CLOSE'] - 0.5 * stock_price_df.at[
            stock_price_df.index[-1], 'ATR']
...
def buy_check(code, stock_price_df, context):
    ...
    macd_code_price_max = max(context.macd_best_atrplus[code][length - context.macd_win_length:])
    if today_price <= macd_code_price_max:
        return False

        # 考虑boll
    if today_price >= stock_price_df.at[stock_price_df.index[-1], 'BOLL_MID']:
        print('价格突破macd和atr界限，处于bolling上半区，买入')
        return True
...
def sell_check(code, stock_price_df, context):
    ...
    if context.portfolio.positions[code].closeable_amount <= 0:
        return False
    # 持有时间短就不卖出
    print('持有天数：', (context.current_dt - context.portfolio.positions[code].init_time).days)
    if (context.current_dt - context.portfolio.positions[code].init_time).days < 5:
        return False
    ...
    #     return False
    # today_price = stock_price_df.at[stock_price_df.index[-1], 'CLOSE']
    # macd_code_price_min=min(context.macd_best_atrmin[code][length-context.macd_win_length:])
    ...
    if stock_price_df.at[stock_price_df.index[-1], 'CLOSE'] < stock_price_df.at[
        ...
        stock_price_df.index[-2], 'BOLL_MID']:
        print(str(context.current_dt.date()) + ' ' + code + " boll条件卖")
        return True

    # # 考虑MA
    # if stock_price_df.at[stock_price_df.index[-1], 'MA5'] <= stock_price_df.at[stock_price_df.index[-1], 'MA20'] and \
    #     stock_price_df.at[stock_price_df.index[-2], 'MA5'] > stock_price_df.at[stock_price_df.index[-1], 'MA20']:
    #     print(str(context.current_dt.date())+' '+code+" MA死叉卖")
    #     return True
