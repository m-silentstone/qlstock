...
def buy_check(code, stock_price_df, context):
    ...
    #     stock_price_df.at[stock_price_df.index[-2], 'MACD'] > 0 and \
    #     stock_price_df.at[stock_price_df.index[-5], 'MACD'] < 0 and \
    #     stock_price_df.at[stock_price_df.index[-4], 'MACD'] < 0 and \
    #     stock_price_df.at[stock_price_df.index[-3], 'MACD'] <= 0:
    if stock_price_df.at[stock_price_df.index[-1], 'MACD'] > 0 and \
            stock_price_df.at[stock_price_df.index[-2], 'MACD'] <= 0 and \
            stock_price_df.at[stock_price_df.index[-3], 'MACD'] < 0:
        if code not in context.macd_best:
            context.macd_best[code] = []
        atr_plus_price = stock_price_df.at[stock_price_df.index[-1], 'CLOSE'] + 0.5 * stock_price_df.at[
            stock_price_df.index[-1], 'ATR']
        context.macd_best[code].append(atr_plus_price)
        n = 2
        if len(context.macd_best[code]) >= n + 1:
            today_price = stock_price_df.at[stock_price_df.index[-1], 'CLOSE']
            macd_best_code = context.macd_best[code][0:n]
            isBuy = True
            for price_in_array in macd_best_code:
                if today_price < price_in_array:
                    isBuy = False
                    break
            context.macd_best[code] = context.macd_best[code][1:]

    # # 考虑成交量
    # if stock_price_df.at[stock_price_df.index[-1], 'VOLUME'] < stock_price_df.at[stock_price_df.index[-1], 'VMA5']:
    #     return False
